...
// ===========================================================================
// NUT container (Phase 5): cross-tool round trips. Two independent NUT
// writers never agree byte-for-byte (frame-code choice, index, elision are
// all free), so fidelity is proven THROUGH the other tool: our NUT demuxed
// by system ffmpeg and vice versa must reproduce the source bit-exactly.
// ===========================================================================
...
/// We mux NUT → system ffmpeg demuxes → y4m byte-equal to the source.
#[test]
fn golden_nut_ours_mux_ffmpeg_demux() {
    let Some(fx) = Fixture::new("nut_out") else {
        eprintln!("skipping: system ffmpeg not found");
        return;
    };
    ...
    assert_eq!(
        ...
        std::fs::read(fx.path("rt.y4m")).unwrap(),
        "ours→NUT→ffmpeg differs from the source"
    );
}

/// System ffmpeg muxes NUT → we demux → y4m byte-equal to the source
/// (exercises our reader against a foreign writer: different frame codes,
/// index bytes, info packets).
#[test]
fn golden_nut_ffmpeg_mux_ours_demux() {
    let Some(fx) = Fixture::new("nut_in") else {
        eprintln!("skipping: system ffmpeg not found");
        return;
    };
    make_nut_source_y4m(&fx);
    ...
    assert_eq!(
        std::fs::read(fx.path("in.y4m")).unwrap(),
        std::fs::read(fx.path("rt.y4m")).unwrap(),
        "ffmpeg→NUT→ours differs from the source"
    );
}

/// Audio both ways through NUT: PCM payload survives byte-exact.
#[test]
fn golden_nut_audio_round_trip_both_ways() {
    let Some(fx) = Fixture::new("nut_audio") else {
        eprintln!("skipping: system ffmpeg not found");
        return;
    };
    fx.run_ffmpeg(&[
